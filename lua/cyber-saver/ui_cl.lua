...
function nlitSaver:CreateUI(toolObj)
  ...
  toolObj:AddControl('Header', {
    Text = '#Tool.' .. self.tool .. '.name',
    Description = '#Tool.' .. self.tool .. '.desc'
  })
  ...
  toolObj:AddControl('CheckBox', {
    Label = l('Place with saving world positions'),
    Command = self.wPosCvar:GetName()
  })

  toolObj:AddControl('CheckBox', {
    Label = l('Freeze Items On Spawn'),
    Command = freezeCvar:GetName()
  })

  toolObj:AddControl('CheckBox', {
    Label = l('Preview'),
    Command = self.previewCvar:GetName()
  })
  ...
  saves.OnRowSelected = function(rowIndex, row) self:ClearClientProps() end
  ...
  AddButton(Buttons:Create('Place saving', function()
    ...
    if self:SaveExists(saveName) then self:SpawnEnts(svs[saveName]) end
  end))
  ...
  AddButton(Buttons:Create('Rename saving', function()
    ...
    if self:SaveExists(saveName) then
      ...
      Frames:AcceptDialogue(l('Rename saving') .. ' ' .. saveName .. ' ' .. l('to') .. ' ' .. newName .. '?', 'Yes', 'No', function()
  ...
  AddButton(Buttons:Create('Clear selection', function() self.Ents = {} end))
  if LocalPlayer():IsSuperAdmin() then
    toolObj:AddControl('Slider', {
      Label = l('Max props') .. ' [' .. l('Admins') .. ']',
      Command = 'sbox_maxprops'
    })
  end
end
